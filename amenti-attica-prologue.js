...
(function () {
  ...
  function style() {
    ...
    s.textContent = [
      ...
      '#amenti-prologue .ap-mast + h4{margin-top:0}',
      ...
      /* the scene view had no title — the reader met a picture with no name
         on it · 12 Sep 2026 */
      /* ── A TITLE BELONGS TO WHAT IT TITLES · 13 Sep 2026 ──────────────────
         Pinned to the top-left corner it was a sign on a wall, a thousand
         pixels from the words it names, with the whole column between them
         doing nothing. It sits at the head of the copy now — the reader meets
         the name, the chapter and the passage in that order, which is the
         order they are read in. */
      '#amenti-prologue .ap-mast{color:#e0913f;font-size:12.5px;',
      '  letter-spacing:.2em;text-transform:uppercase;margin-bottom:14px;',
      '  padding-bottom:12px;border-bottom:1px solid rgba(224,145,63,.22)}',
      ...
      /* ── THE CAPTION WAS PRINTED THROUGH MOUNT ATHOS · 13 Sep 2026 ───────
         A strip at the foot of the pane covers the foot of the pane, and the
         wreck is at the foot of the pane. It sits under the picture instead —
         a caption is about the thing, not part of it. */
      '#amenti-prologue .ap-incap{color:#7d8ea6;font-size:9px;',
      '  letter-spacing:.05em;line-height:1.5;margin-top:-2px;padding:0 2px}',
      ...
      /* ── IT WAS TOO DARK TO READ · 13 Sep 2026 ───────────────────────────
         Half opacity on a night relief is a grey field. The chart has one job
         — show how far Susa is — and a reader could not see the ground between
         them. Brighter, with the contrast lifted, and the theatre box filled
         rather than outlined so the eye finds it first. */
      '#amenti-prologue .ap-far img{position:absolute;inset:0;width:100%;',
      '  height:100%;opacity:.9;filter:brightness(1.35) contrast(1.15)}',
      '#amenti-prologue .ap-far .ap-far-box{position:absolute;',
      '  border:1.5px solid rgba(224,145,63,.95);border-radius:1px;',
      '  background:rgba(224,145,63,.16);',
      '  box-shadow:0 0 0 1px rgba(5,8,14,.5)}',
      ...
      '#amenti-prologue .ap-far .ap-far-mk s{position:absolute;left:7px;',
      '  top:-6px;text-decoration:none;white-space:nowrap;color:#cfdbe8;',
      '  font-size:8.5px;letter-spacing:.06em;',
      '  text-shadow:0 1px 4px rgba(0,0,0,1),0 0 6px rgba(0,0,0,.9)}',
      ...
      '  background:linear-gradient(180deg,rgba(7,13,22,0),rgba(7,13,22,.9) 55%)}',
  ...
  function bearings() {
    ...
    /* ── THE MARK WAS A PLACE AND THE POINT IS A MAN · 13 Sep 2026 ─────
       `Susa` is a city a reader may not know. What the chart is for is the
       distance between the man giving the order and the ground it falls on,
       so the mark says who is there. */
  ...
  function show(n) {
    ...
    /* ── TWO ELEMENTS, ONE CLASS · 12 Sep 2026 ──────────────────────────
       ...
       again. */
    asL.innerHTML = '<div class="ap-mast">The Attica Campaign</div>' +
      '<h4>' + esc(yr(s.year)) + '  \u00b7  Herodotus ' +
  ...
  function locator(s) {
    ...
    if (IN) {
      ...
      cap.className = 'ap-incap';   /* below the pane, not over it */
    ...
    /* ── THE BLOCK WAS PASTED, NOT MOVED · 11 Sep 2026 ────────────────────
      ...
      /* ── A SEA LEG DRAWS AS WATER · 13 Sep 2026 ──────────────────────
         The key was given a sine and the line stayed dashed, so the map's own
         legend described a mark the map did not draw. I CALLED IT AN IDIOM AND
         LET IT STAND, which is painting the mirror: the fix belonged on the
         line.

         The polyline is resampled at a fixed step and pushed perpendicular on
      ...
      /* ── A LINE THAT RUNS INTO ITS OWN MARKS · 13 Sep 2026 ───────────
         The leg started inside the departure mark and ended inside the
         arrival one, so both ends were a tangle of line, dot and glyph — and
         at Ephesos the arrowhead came to rest ON THE COAST, which is the one
         thing a fleet may not do.

         Both ends are cut back. The reader has the departure and the arrival
         as marks already; the line is the passage between them and does not
         need to touch either. Trimmed, the head falls in open water where the
         last leg actually ran. */
      function trim(list, head, tail) {
        var seg = [], total = 0, i;
        for (i = 0; i < list.length - 1; i++) {
          var d = Math.hypot(list[i + 1].x - list[i].x, list[i + 1].y - list[i].y);
          seg.push(d); total += d;
        }
        if (total < (head + tail) * 2.2) { return list; }
        function at(dist) {
          var run = 0;
          for (var j = 0; j < seg.length; j++) {
            if (run + seg[j] >= dist) {
              var t = seg[j] ? (dist - run) / seg[j] : 0, a = list[j], b = list[j + 1];
              return { x: a.x + (b.x - a.x) * t, y: a.y + (b.y - a.y) * t };
            }
            run += seg[j];
          }
        ...
        var out = [at(head)], run2 = 0;
        for (i = 0; i < seg.length; i++) {
          run2 += seg[i];
          if (run2 > head && run2 < total - tail) { out.push(list[i + 1]); }
        }
        out.push(at(total - tail));
        return out;
      }
      ...
      /* ── TWO ARRIVALS IN ONE PLACE · 13 Sep 2026 ─────────────────────
         ...
         is the same statement twice — and the larger of the two, drowning the
         thing it duplicates. THE HULLS ARE THE ARRIVAL. A road keeps its head,
         because a road has nothing else to end with. */
      var p1 = draw[draw.length - 2], p2 = draw[draw.length - 1];
      var ang = Math.atan2(p2.y - p1.y, p2.x - p1.x) * 180 / Math.PI;
      var hd = (kind === 'sea') ? null :
      ...
      /* the box is far wider than it is tall and `preserveAspectRatio` is
         `none`, so every unit of x is worth more than a unit of y — a head
         that reads as small in these numbers still draws large · 13 Sep */
      if (hd) {
        hd.setAttribute('d', thin ? 'M0,0 L-1.1,0.42 L-1.1,-0.42 Z'
                                  : 'M0,0 L-1.5,0.72 L-1.5,-0.72 Z');
      ...
      /* ── THE FLEET IS WHERE IT PUT IN · 13 Sep 2026 ──────────────────
         Hulls spaced along the whole leg drew a fleet strung across the
         Aegean, which is not a thing that happened — it says the ships were
         everywhere at once. THEY GATHER AT THE ARRIVAL, offshore, where the
         passage leaves them: twenty Athenian ships and five Eretrian, put in
         on the Ionian coast, and the men go up the road from there.

         IT IS NOT A COUNT. Five hulls because five read as a squadron at this
         size, not because five ships did anything. Spread across the line's
         own direction so the group faces the shore it came to.
      ...
      if (kind === 'sea' && draw.length > 3) {
        ...
        [[-1.1, -1.5], [-2.4, 0.4], [-1.4, 1.7], [-3.9, -0.9], [-3.6, 1.4]]
          .forEach(function (o, i) {
            ...
            sh.setAttribute('fill', hue);
            sh.setAttribute('opacity', (0.82 - i * 0.07).toFixed(2));
            sh.setAttribute('transform', 'translate(' + hx.toFixed(2) + ' ' +
                            hy.toFixed(2) + ') rotate(' + (ang + (i % 2 ? 7 : -6)) + ')');
            sv.appendChild(sh);
          });
      }
      host.appendChild(sv);
