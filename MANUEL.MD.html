...
*A glossary of every named artifact in Page 2, drawn from the working code as of v9.18.62.*
...
The Live Activity terminal beneath the deck streams events from every angel in real time, color-coded by sigil tag (MIC, GAB, URI, CAS, HER, ORG, RAM, MAN, SYS).
...
Right half of the chamber, accessed by clicking the second sigil. Materializes a structured fragment of the figure's record — a primary-source-style article presentation with header, biography, and link cluster. Powered by Hermes, who fetches the structured profile (events, works, affiliations, contemporaries) from Claude on demand and caches it for the browser session.
...
Computes the natal-style astrological chart for any year in the manifold. Cassiel is the deterministic angel: given a year, he returns the same chart every time, because the underlying astronomy is deterministic.
...
Generates the Akasha Archive view. Where Gabriel handles conversational chat in the figure's voice, Hermes asks Claude for a *structured* profile of the figure — events, works, affiliations, contemporaries — and returns it as a JSON object that the UI renders into the archive page.
...
Owns operator-tunable configuration that should not live in source. Where `AMENTI_CONFIG` carries values an operator sets at install time (the canonical CSV URL, the MANUEL.md path), Origin carries values an operator changes at runtime: the AI proxy URL, the chosen Claude model, the conversation history cap.
...
4. `Sovereign.QuantumContainer.init()` — creates scene, camera, renderer, and every visible 3D object.
5. `animate()` — begins the per-frame render loop.
6. `Sovereign.Angels.Michael.reload()` — auto-fetches the canonical ledger.
...
| Akasha Archive | Hermes | Structured profile generation |
| (no module — chart consumer) | Cassiel | Celestial reckoning |
...
Modules without a governing angel are handled directly in `Sovereign.UI.*` and `Sovereign.Navigation.*`. They could be assigned angels later if those subsystems grow in complexity. Cassiel and Ramiel intentionally have no governing module: Cassiel is consumed by the tablet's chart display, and Ramiel runs at boot rather than being user-invoked.
