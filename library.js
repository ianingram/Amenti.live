...
(function () {
  ...
  var VOICE_REGISTER = 'Read slowly and clearly, in a measured, formal, dignified tone';
  ...
  //   PLAYBACK_RATE : words inside every chunk, compressed uniformly (pitch-safe)
  ...
  var PLAYBACK_RATE = 1.12;  // ~12% faster; lifts pace without sounding rushed
  var REST_SOFT     = 0.18;  // seconds
  var REST_SENTENCE = 0.42;  // seconds
  var REST_PARA     = 0.85;  // seconds
  ...
  function chunkText(text, maxChars) {
    ...
    for (var p = 0; p < paragraphs.length; p++) {
      ...
      // tag each piece with its rest; only the paragraph's last piece gets REST_PARA
      for (var k = 0; k < pieces.length; k++) {
        var endsPara = (k === pieces.length - 1);
        chunks.push({ text: pieces[k], rest: restFor(pieces[k], endsPara) });
      }
    }
    return chunks;
  }
  ...
  function startReading(text, btn, style, voice) {
    ...
    var useStyle = style || VOICE_REGISTER;
    ...
    function scheduleBuf(buf, rest) {
      if (player.cancelled) return;
      var src = ctx.createBufferSource();
      src.buffer = buf;
      // Compress the words inside the chunk for a human pace (pitch-preserving on
      // browsers that honor it; a uniform rate, so it adds no chunk-to-chunk drift).
      try { src.playbackRate.value = PLAYBACK_RATE; } catch (e) {}
      ...
      var played = buf.duration / (PLAYBACK_RATE || 1);
    ...
    function drain() {
      while (Object.prototype.hasOwnProperty.call(player.ready, player.toSchedule)) {
        ...
        player.toSchedule++;
        if (entry && entry.buf) scheduleBuf(entry.buf, entry.rest);
      ...
      while (!player.cancelled && inflight < cap && next < chunks.length) {
        (function (idx) {
          ...
          fetchChunkBytes(chunks[idx], useStyle, useVoice, player.abort ? player.abort.signal : null)
            ...
            .then(function (buf) {
              if (!player.cancelled) { player.ready[idx] = buf ? { buf: buf, rest: chunks[idx].rest } : null; drain(); }
              done();
            }, function (err) {
              if (!player.cancelled) {
  ...
  function speakReading(text, btn, figureName) {
    try {
      ...
      resolveVoice(figureName).then(function (v) {
        ...
        startReading(text, btn, VOICE_REGISTER, VOICE_NAME_DEFAULT);
      });
    } catch (e) {
