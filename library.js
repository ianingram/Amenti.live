...
(function () {
  ...
  var VOICE_STYLE  = 'Read slowly and clearly, in a measured, formal, dignified tone';
  var VOICE_NAME   = 'Kore';
  ...
  // Strip markdown to clean prose so the model reads words, not symbols.
  ...
  // Deterministic chunker. plainText() has already collapsed whitespace, so there
  // are no paragraph breaks to split on — pack whole sentences up to CHUNK_MAX,
  // hard-splitting (at word boundaries) only a lone sentence that exceeds it.
  ...
  function chunkText(text, maxChars) {
    var sentences = splitSentences(text), chunks = [], cur = '';
    for (var i = 0; i < sentences.length; i++) {
      var s = sentences[i];
      if (s.length > maxChars) {
        if (cur) { chunks.push(cur); cur = ''; }
        var pieces = hardSplit(s, maxChars);
  ...
  // Fetch one chunk's WAV bytes from the Worker. Retries the known transient where
  // Gemini returns text instead of audio (/speak -> 502 {error:"no_audio"}), so a
  // single flaky chunk doesn't leave a silent gap in the reading.
  function fetchChunkBytes(chunk, signal) {
    var attempts = 0;
    function go() {
      ...
      return fetch(VOICE_WORKER, {
        ...
        body: JSON.stringify({ text: chunk, style: VOICE_STYLE, voice: VOICE_NAME }),
  ...
  // Begin a chunked, gap-free reading of `text` on `btn`. Assigns voicePlayer.
  function startReading(text, btn) {
    ...
    function scheduleBuf(buf) {
      ...
      src.start(at);
      player.nextStart = at + buf.duration;
      player.scheduled++;
    ...
    function drain() {
      while (Object.prototype.hasOwnProperty.call(player.ready, player.toSchedule)) {
        var b = player.ready[player.toSchedule];
    ...
    function pump() {
      while (!player.cancelled && inflight < cap && next < chunks.length) {
        (function (idx) {
          inflight++;
          fetchChunkBytes(chunks[idx], player.abort ? player.abort.signal : null)
            .then(function (bytes) {
              if (player.cancelled) return null;
              return decodeAudio(ctx, bytes);
            })
            .then(function (buf) {
              if (!player.cancelled) { player.ready[idx] = (buf || null); drain(); }
              done();
            }, function (err) {
              if (!player.cancelled) {
  ...
  // different one to switch readings.
  function speakReading(text, btn) {
    try {
  ...
  function openWork(w, catalog, firstName, bodyEl, btn) {
    ...
    fetchBody(w.file).then(function (md) {
      ...
      if (raBtn) raBtn.addEventListener('click', function () { speakReading(md, raBtn); });
      reveal(bodyEl, btn, w);
    })['catch'](function (err) {
