...
**The prefix is the folder** — `Amenti-Technical-Briefs` is flat, and the
filename does the sorting. A document's kind is legible wherever it appears —
in a catalogue entry, in a link, in a search result — without a directory to
explain it. Added 4 Sep, when `FINDING-` became the fifth.

| prefix | what it is | how it ends |
|---|---|---|
| `BRIEF-` | an argument. How something works and why it was built that way. | a position |
| `RULING-` | a decision. **This, not that** — and what it costs. | a choice, and its price |
| `FINDING-` | what was **measured**. No thesis and no decision: the counts, and what they turned out to mean. | a fact |
| `BUILD-` | a plan for something not yet made. | a thing to do |
| `HANDOFF-` | the state of the ship at the end of a session. | where the next hand starts |

> **A FINDING HAS NOTHING TO WIN.**
> A brief argues and a ruling decides. A finding only reports what the
> instrument said — including, and especially, when the obvious cause was
> present, was guilty of something else, and was not guilty of this.

The first is `FINDING-THE-SQUARING-OF-THE-LAKES`, 4 Sep: one appearance —
angular inland water — that measurement split into a source-scale artefact, two
separate bugs in one simplification pass, an invalid measurement built on top of
one of them, and a real feature of the world. Fifty-six of the lakes were
reservoirs, and their squared outlines are the record rather than an error: a
dam drowns a valley and takes its shape.
