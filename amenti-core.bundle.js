/* ============================================================================
   ...
   ----------------------------------------------------------------------------
   SIX FILES, ONE ROUND TRIP. Nothing here is edited — this is a concatenation
   in the exact order the page loaded them, and it must stay that way:
       ...
       amenti-quiz.js      the quiz client

   WHY THIS IS SAFE, CHECKED RATHER THAN ASSUMED
     · every one of the six is fully IIFE-wrapped — 0 top-level declarations
       between them, so merging them into one script scope cannot collide
     · none carries 'use strict' at file scope, so none can impose it on the
       others by being merged
     · none reads document.currentScript, which is the usual thing that breaks
       when a file stops being its own file
     · concatenation preserves execution order exactly, which is what the
       parse-time consumers in Page1.html depend on

   WHY IT IS WORTH DOING
     These six are the last render-blocking scripts on the page. The bytes were
     never the problem — six SERIAL round trips were, each halting the parser
     until it returned. One file, same bytes, one trip.

   IF YOU EDIT ANY SOURCE FILE, REBUILD THIS. It is generated, not authored.
   ============================================================================ */
