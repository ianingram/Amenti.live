...
def load():
    ...
    for r in csv.DictReader(open(NAMES, encoding='utf-8')):
        if r['searchable'] == 'yes':
            ...
            # ── AN ATTESTED HIT AND A DERIVED HIT ARE DIFFERENT EVIDENCE ────
            # ATTICA-NAMES.csv gained an `origin` column on 8 Sep: `attested`
            # for a form Pleiades records, `derived` for one generated by
            # tools/latinise-names.py. 1,670 of the latter took the named
            # places from 152 to 236.
            #
            # A hit on an ATTESTED form says the corpus names this place.
            # A hit on a DERIVED form says the corpus names something that
            # TRANSLITERATES to this place. Both are worth having and they are
...
def main():
    ...
    if dv:
        print()
        print('  %d of the %d rest ENTIRELY ON DERIVED FORMS \u2014 a transliteration'
              % (len(dv), len(named)))
        print('  matched, not a spelling anyone recorded. They are real hits and')
        print('  they are weaker evidence, and a reader must be able to see which.')
        for r in sorted(dv, key=lambda x: -x['sources'])[:8]:
            print('    %-32s %2d src  as %s' % (r['name'][:32], r['sources'],
                                                r['matched_as'][:24]))
    print()
