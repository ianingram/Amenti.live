...
/* ── RULE 2 · ATTRIBUTE, NEVER INFER ─────────────────────────────────────
   THE PROBE CORPS doctrine, and this function broke it on its first draft:

     return r.ok ? await r.text() : null;

   A 404, a 500, a rate-limit and a thrown socket error all became `null`, and
   the report said only "could not be fetched". That is a CATEGORY inferred in
   place of a CAUSE — the same fault the doctrine's second rule was written
   from, where a 400 caused by the probe's OWN malformed request was reported
   as the wall breaking conversations.

   A file that 404s has been RENAMED OR REMOVED and is a finding.
   A file that 500s or rate-limits is the SOURCE being unavailable and is not.
   Reporting them alike would send somebody looking for a deleted page that is
   sitting exactly where it always was.

   So it returns { text, why } and the reason travels with the failure. */
async function readFile(repo, branch, p) {
  const url = `${RAW}${OWNER}/${repo}/${branch}/${p}?_=${BUST}`;
  try {
    const r = await fetch(url, { cache: 'no-store', headers: { 'Cache-Control': 'no-cache' } });
    if (r.ok) return { text: await r.text(), why: 'ok' };
    return { text: null, why: 'HTTP ' + r.status +
      (r.status === 404 ? ' — renamed or removed' : ' — the source, not the file') };
  } catch (e) {
    return { text: null, why: 'fetch threw: ' + ((e && e.message) || String(e)) };
  }
}
...
(async () => {
  ...
  for (const w of (sem.walk || [])) {
    ...
    for (const f of files) {
      const got0 = await readFile(w.repo, w.branch, f);
      const html = got0.text;
      /* EMPTY GLASS, and RULE 2. A file that could not be read is NAMED with
         its REASON, never silently skipped and never lumped — otherwise a
         network blip reads as a repo with no surfaces, and a deleted page
         reads the same as a rate limit. */
      if (html === null) { unread.push({ path: `${w.repo}/${f}`, why: got0.why }); continue; }
  ...
  if (unread.length) {
    say(`UNREAD        ${unread.length} file(s) could not be read:`);
    unread.forEach(u => say(`              ${u.path}  \u2014 ${u.why}`));
  }
