...
> `logs/2026-08-27.md` — **the first of the series.** One file per session, in
> `logs/`, named date-first so it sorts. A folder rather than one growing
> document: a single file spanning fifty sessions puts every diff on one blob
> and lets one bad paste damage every log at once. **A log is an account of a
> day, and a day does not change.**
